...
object HealthChecker {
    ...
    // Device age in years, minimum 0
    private fun calculateDeviceAge(): Int {
        val buildTime = Build.TIME // firmware build time
        val currentTime = System.currentTimeMillis()
        val ageInYears = ((currentTime - buildTime).toDouble() / (1000 * 60 * 60 * 24 * 365)).toInt()
        return ageInYears.coerceAtLeast(0)
    }
    ...
    private fun getRAMUsagePercent(context: Context): Int {
        ...
        return ((used.toDouble() / memoryInfo.totalMem) * 100).toInt().coerceIn(0, 100)
    }
    ...
    fun calculateUpdateReadiness(health: DeviceHealth): UpdateReadiness {
        ...
        // Device age penalties
        when (health.deviceAgeScore) {
            0, 1 -> { /* New device, no penalty */ }
            2 -> { score -= 15; suggestions.add("Device is 2 years old; minor caution advised") }
            3 -> { score -= 30; suggestions.add("Device is 3 years old; consider updating apps first") }
            4 -> { score -= 45; suggestions.add("Device is 4 years old; update only essential apps") }
            else -> { score -= 60; suggestions.add("Device is 5+ years old; updating may cause issues") }
        }
